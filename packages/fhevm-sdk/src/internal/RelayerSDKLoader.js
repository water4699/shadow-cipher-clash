...
class RelayerSDKLoader {
    ...
    load() {
        ...
        return new Promise((resolve, reject) => {
            const existingScript = document.querySelector(`script[src="${constants_1.SDK_CDN_URL}"]`);
            if (existingScript) {
            ...
            script.onload = () => {
                if (!isFhevmWindowType(window, this._trace)) {
                    ...
                    reject(new Error(`RelayerSDKLoader: Relayer SDK script has been successfully loaded from ${constants_1.SDK_CDN_URL}, however, the window.relayerSDK object is invalid.`));
                }
                resolve();
            };
            script.onerror = () => {
                console.log("[RelayerSDKLoader] script onerror... ");
                reject(new Error(`RelayerSDKLoader: Failed to load Relayer SDK from ${constants_1.SDK_CDN_URL}`));
            };
